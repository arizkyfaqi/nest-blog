...
describe('UsersService', () => {
  ...
  beforeEach(async () => {
    const mockCreateUserPtovider: Partial<CreateUserProvider> = {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        UsersService,
        { provide: CreateUserProvider, useValue: mockCreateUserPtovider },
  ...
  describe('createUser', () => {
    ...
    it('Should call createUser on CreateUserProvider', async () => {
      let user = await service.createUser({
        firstName: 'Jhone',
        lastName: 'China',
        email: 'jhonecina@example.com',
        password: 'password',
      });
      expect(user.firstName).toEqual('Jhone');
    });
  });
});
